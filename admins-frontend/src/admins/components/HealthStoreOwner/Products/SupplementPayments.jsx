...
const SupplementPayments = () => {
  ...
  const fetchTransactions = useCallback(async () => {
    ...
    try {
      ...
      // Filter orders that contain at least one Supplement item
      const supplementOrders = allOrders.filter(order =>
        order.items?.some(item => item.productType === 'Supplement')
      );

      setTransactions(supplementOrders);
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h2 className="text-xl font-bold text-gray-800">Supplement Payment Ledgers</h2>
          <p className="text-slate-500 text-xs mt-0.5">Audit transaction history, payment verification IDs, and settlement accounts for supplements.</p>
        </div>
...
export default SupplementPayments;
