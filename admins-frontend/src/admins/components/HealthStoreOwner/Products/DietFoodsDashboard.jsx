...
const DietFoodsDashboard = () => {
  ...
  useEffect(() => {
    const fetchDietStats = async () => {
      setLoading(true);
      try {
        const res = await getProducts({ type: 'Diet', limit: 100 });
        const allDietFoods = res.data.data || [];
        
        const computedStats = allDietFoods.reduce(
          (acc, item) => {
            acc.total += 1;
          ...
          { total: 0, live: 0, pending: 0, draft: 0, rejected: 0 }
        );

        setStats(computedStats);
        setRecentItems(allDietFoods.slice(0, 3));
      } catch (err) {
        toast.error('Failed to load diet plans statistics');
      } finally {
        setLoading(false);
      }
    };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
        <div className="bg-gradient-to-br from-slate-900 to-slate-850 text-white rounded-2xl p-5 shadow-sm border border-slate-800">
          <p className="text-slate-400 text-xs font-bold uppercase tracking-wider">Total Diet Plans</p>
        ...
        <div className="bg-white border rounded-2xl p-5 shadow-sm">
          <p className="text-gray-400 text-xs font-semibold uppercase tracking-wider">Draft Plans</p>
          <p className="text-3xl font-extrabold text-gray-700 mt-2">{stats.draft}</p>
          <span className="text-[10px] text-gray-500 bg-gray-50 px-2 py-0.5 rounded-full mt-2 inline-block font-bold">In Draft</span>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 bg-white border rounded-2xl p-6 shadow-sm">
          <div className="flex justify-between items-center mb-4">
            <h3 className="font-bold text-gray-850 text-base">Recently Added Diet & Food Plans</h3>
            <button onClick={() => navigate('/health-store-owner/diet-foods/list')} className="text-xs font-bold text-red-600 hover:text-red-750">
              View All Listings &rarr;
            </button>
          </div>

          {recentItems.length === 0 ? (
            <div className="p-8 text-center text-gray-400 text-sm">No diet plans added yet.</div>
          ) : (
                ...
                <div key={item._id} className="py-4 flex justify-between items-center gap-4">
                  <div className="flex items-center gap-3">
                    {item.images?.[0] ? (
                      ...
                      <div className="w-12 h-12 rounded-lg bg-red-50 flex items-center justify-center text-red-655 text-xl font-bold border">
                        🥗
                      </div>
                    )}
                    <div>
        ...
        <div className="bg-white border rounded-2xl p-6 shadow-sm space-y-4">
          ...
          <div className="space-y-3">
            <button
              onClick={() => navigate('/health-store-owner/diet-foods/add')}
            ...
            >
              <div>
                <p className="font-bold text-gray-800 text-xs">🥗 Add New Diet/Food</p>
                <p className="text-[9px] text-gray-400 mt-0.5">List weight loss/gain diet tables</p>
              </div>
              <span className="text-slate-400 font-bold">&rarr;</span>
            </button>

            <button
              onClick={() => navigate('/health-store-owner/diet-foods/list')}
              className="w-full flex items-center justify-between bg-slate-50 hover:bg-slate-100 p-3 rounded-xl border transition-all text-left"
            >
              <div>
                <p className="font-bold text-gray-800 text-xs">📋 View Diet List</p>
                <p className="text-[9px] text-gray-400 mt-0.5">Edit status, prices, and settings</p>
              </div>
              <span className="text-slate-400 font-bold">&rarr;</span>
            </button>
          </div>
...
export default DietFoodsDashboard;
