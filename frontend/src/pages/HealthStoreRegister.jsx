...
import { validateInviteToken, registerHealthStore } from '../userServices/publicHealthStoreApi';
  ...
  useEffect(() => {
    const checkToken = async () => {
      try {
        ...
        toast.error(err.message || 'Invalid or expired invite token');
      } finally {
        setChecking(false);
      }
    };
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
              <div>
                ...
                <input required type="file" accept="image/*" onChange={e => setLogo(e.target.files[0])}
                  className="w-full text-xs text-zinc-550 file:mr-4 file:py-2 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-semibold file:bg-zinc-800 file:text-white hover:file:bg-zinc-700" />
              </div>
              <div>
